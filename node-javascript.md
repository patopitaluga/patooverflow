...
forEach: run a function for every element of an array. Useful to set existing variables or run a function for every element.
```
...
Map: Get an array with transformations of every element in other array.
```
let people = [{ name: 'a' }, { name: 'b' }];
...
Filter: Get an array with transformations of every element in other array.
```
let people = [{ name: 'a' }, { name: 'b' }];
let greetings = people.filter((eachOne, index, theArray) => { // returns an array with each return as an element in the array.
  return true; // should be returning a boolean
});
console.log(greetings);
```
